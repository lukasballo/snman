...
def graph_from_bbox(
    ...
    G = graph_from_polygon(
        ...
        simplify=simplify,
        simplify_strict=simplify_strict,
...
def graph_from_polygon(
    ...
    if clean_periphery:
        ...
        if simplify:
            G_buff = simplification.simplify_graph(G_buff, strict=simplify_strict)
    ...
    # if clean_periphery=False, just use the polygon as provided
        ...
        bidirectional = network_type in settings.bidirectional_network_types
        G = _create_graph(response_jsons, retain_all=True, bidirectional=bidirectional, one_edge_per_direction=one_edge_per_direction)
        ...
        if simplify:
            G = simplification.simplify_graph(G, strict=simplify_strict)
...
def graph_from_xml(filepath, bidirectional=False, simplify=True, simplify_strict=True, retain_all=False):
    """
...
def _create_graph(response_jsons, retain_all=False, bidirectional=False, one_edge_per_direction=True):
    """
...
def _add_paths(G, paths, bidirectional=False, one_edge_per_direction=True):
    """
    ...
    for path in paths:
        ...
        if not is_one_way and one_edge_per_direction:
            path["reversed"] = True
